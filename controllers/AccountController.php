...
class AccountController extends Controller{//登录、注册、找回账户等等
								...
								[
									'actions' => ['captcha','ceshi','teacher-login','register','login','admin-login','register2',
                                            ...
                                            'ajax-set-newpassword','teacher-logout','admin-logout','choice-login','email-reset-userpwd',
                                             'ajax-send-adminpwd-email','email-reset-adminpwd','forget-adminpwd-email'
									],
									'allow' => true,
								],
								[
	...
	public function actionAdminLogin(){//管理员登录
	...
	public function actionForgetUserpwdEmail(){ //学生找回密码跳转
        return $this->render("forget-userpwd-email");
    }

    public function actionForgetAdminpwdEmail(){ //管理员找回密码跳转
        return $this->render("forget-adminpwd-email");
    }

	public function actionAjaxSendPasswordEmail(){ //老师邮件找回密码
		$email=$_POST['email'];
    ...
    public function actionEmailResetUserpwd($email,$token){//点击验证邮箱
    ...
    public function actionEmailResetAdminpwd($email,$token){//点击验证邮箱
        $mail=base64_decode($email);
        $token=base64_decode($token);
	...
	public function actionEmailResetPassword($email,$token){//点击验证邮箱
		$mail=base64_decode($email);
		$token=base64_decode($token);
	...
	public function actionAjaxSetNewpassword(){//找回密码时候设置新密码保存
	...
	public function actionSendMail(){//发送邮件
